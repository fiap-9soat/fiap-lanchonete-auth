...
role_arn   = "arn:aws:iam::123291832101:role/LabRole"
cognito_domain = "minha-url-cognito" # deve ser unica, fiap-lanchonete-auth-1 caso omitido.
```

_Atenção: essas credenciais são inválidas, e servem apenas como exemplo. Você deve obter as credenciais corretas do
próprio ambiente da AWS._

Caso você utilize o `AWS CLI`, os parametros no arquivo `~/.aws/credentials` podem ser utilizados para autenticação.  
O `role_arn` pode ser obtido de alguma "Role" no `AWS IAM`. Vale ressaltar que a Role deve ter os acessos necessários
